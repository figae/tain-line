...
export const description =
  "12 characters, 5 places, 7 events across all major cycles. Broad but shallow.";
...
export const seed: Seed["seed"] = (db) => {
  ...
  const e_firstBattle = insertEvent.run(
    ...
    "mythological", "Age of Gods", cmtSrc
  ).lastInsertRowid as number;
  ...
  const e_lirChildren = insertEvent.run(
    ...
    "mythological", "Age of Gods", lirSrc
  ).lastInsertRowid as number;
  insertEC.run(e_lirChildren, lir, "mentioned", "Father of the transformed children", lirSrc);

  // ── EVENT DEPENDENCIES ─────────────────────────────────────────────────
  insertDep.run(e_firstBattle, e_silverArm, "Nuada lost his arm at the First Battle, which necessitated the silver replacement", "certain", cmtSrc);
  insertDep.run(e_silverArm, e_lughArrives, "Nuada's restored kingship is the political context for Lugh's arrival at Tara", "probable", cmtSrc);
  insertDep.run(e_firstBattle, e_secondBattle, "The two battles are sequential — First Battle of Mag Tuired precedes the Second", "certain", cmtSrc);
  insertDep.run(e_lughArrives, e_secondBattle, "Lugh must have established himself at Tara before leading the Tuatha Dé Danann to war", "certain", cmtSrc);
  insertDep.run(e_secondBattle, e_nuadaDies, "Nuada's death occurs during the Second Battle of Mag Tuired", "certain", cmtSrc);
  insertDep.run(e_silverArm, e_nuadaDies, "Nuada receives his arm before the battle in which he dies", "certain", cmtSrc);
  insertDep.run(e_secondBattle, e_tain, "The Ulster Cycle takes place in a later age than the Mythological Cycle", "probable", cmtSrc);
};
